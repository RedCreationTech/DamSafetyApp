...
def condense_release(k, m, end, code='allm'):
    if code == 'bend':
        rel = [10, 11] if end == 's2' else [4, 5]
    else:
        rel = [9, 10, 11] if end == 's2' else [3, 4, 5]
    keep_rel_marker = True
...
def main():
    ...
    release_map = {rel['geid']: (rel['end'], rel.get('code', 'allm'))
    ...
    for bname, conn in sorted(blocks.items()):
        ...
        for e in conn:
            ...
            rc = release_map.get(geid)
            if rc:
                ke, me, keep = condense_release(ke, me, rc[0], rc[1])
                end = rc[0]
                n_hinge += 1
            else:
                keep = list(range(12))
